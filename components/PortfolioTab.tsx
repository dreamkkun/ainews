...
export default function PortfolioTab() {
  ...
  const [form, setFormData]   = useState({ companyName: "", ticker: "", purchasePrice: "", currentPrice: "", highestPrice: "" });
  const [editId, setEditId]   = useState<string | null>(null);

  useEffect(() => { setStocks(loadStocks()); }, []);

  const calcStock = (s: Omit<TrackedStock, "status" | "dropFromPeak" | "lossFromPurchase" | "bufferToStopLoss">): TrackedStock => {
    const drop   = ((s.highestPrice - s.currentPrice) / s.highestPrice) * 100;
    const loss   = ((s.currentPrice - s.purchasePrice) / s.purchasePrice) * 100;
    const buffer = 20 - drop;
    const status: TrackedStock["status"] = drop >= 20 ? "Cut-Loss" : loss >= 20 ? "Take-Profit" : "Hold";
    return { ...s, status, dropFromPeak: drop, lossFromPurchase: loss, bufferToStopLoss: buffer, updatedAt: new Date().toISOString() };
  };
  ...
  const save = () => {
    ...
    const next = editId
      ? stocks.map(s => s.id === editId ? stock : s)
      : [...stocks, stock];
    setStocks(next); saveStocks(next);
    setForm({ companyName: "", ticker: "", purchasePrice: "", currentPrice: "", highestPrice: "" });
    setForm({ companyName: "", ticker: "", purchasePrice: "", currentPrice: "", highestPrice: "" });
    setEditId(null); setForm({ companyName: "", ticker: "", purchasePrice: "", currentPrice: "", highestPrice: "" }); setEditId(null); setForm({companyName:"",ticker:"",purchasePrice:"",currentPrice:"",highestPrice:""}); setEditId(null); setForm({companyName:"",ticker:"",purchasePrice:"",currentPrice:"",highestPrice:""}); setEditId(null);
    setForm({companyName:"",ticker:"",purchasePrice:"",currentPrice:"",highestPrice:""}); setEditId(null); setForm({companyName:"",ticker:"",purchasePrice:"",currentPrice:"",highestPrice:""}); setEditId(null);
    setForm({companyName:"",ticker:"",purchasePrice:"",currentPrice:"",highestPrice:""}); setEditId(null);
    setForm({companyName:"",ticker:"",purchasePrice:"",currentPrice:"",highestPrice:""}); setEditId(null);
    setForm({companyName:"",ticker:"",purchasePrice:"",currentPrice:"",highestPrice:""}); setEditId(null);
    setShowForm_clean(next);
  };

  const setShowForm_clean = (next: TrackedStock[]) => {
    setStocks(next); saveStocks(next);
    setForm({companyName:"",ticker:"",purchasePrice:"",currentPrice:"",highestPrice:""});
    setEditId(null); setForm({companyName:"",ticker:"",purchasePrice:"",currentPrice:"",highestPrice:""});
    setEditId(null);
    setForm({companyName:"",ticker:"",purchasePrice:"",currentPrice:"",highestPrice:""});
    setEditId(null);
    setForm({companyName:"",ticker:"",purchasePrice:"",currentPrice:"",highestPrice:""});
  ...
  const startEdit = (s: TrackedStock) => {
    setEditId(s.id);
    setFormData({ companyName: s.companyName, ticker: s.ticker ?? "", purchasePrice: String(s.purchasePrice), currentPrice: String(s.currentPrice), highestPrice: String(s.highestPrice) });
    setForm(true);
  };
  ...
  return (
    <div style={{ padding: "24px 32px", maxWidth: 1280, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, paddingBottom: 8, borderBottom: "1px solid #21262D" }}>
        ...
        <button onClick={() => { setShowForm(!showFormState); setEditId(null); setFormData({companyName:"",ticker:"",purchasePrice:"",currentPrice:"",highestPrice:""}); }} style={btnAdd}>
          {showFormState ? "✕ 닫기" : "+ 종목 추가"}
        </button>
      </div>

      {/* 추가/수정 폼 */}
      {(showFormState || editId) && (
        <div style={{ background: "#161B22", border: "1px solid #30363D", borderRadius: 12, padding: "20px 24px", marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={() => { setShowForm(false); setEditId(null); }} style={btnCancel}>취소</button>
          </div>
        </div>
      )}
      ...
      <p style={{ color: "#8B949E", fontSize: "0.75rem", textAlign: "center", marginTop: 24 }}>※ 손절 기준: 고점 대비 -20% / 익절 기준: 매수가 대비 +20%</p>
    </div>
  );
}
