...
interface Settings { stopLossPct: number; takeProfitPct: number; }

const EMPTY = { companyName: "", ticker: "", purchasePrice: "", currentPrice: "", highestPrice: "" };
const DEFAULT_SETTINGS: Settings = { stopLossPct: 20, takeProfitPct: 20 };
...
export default function PortfolioTab() {
  ...
  const calcStock = (
    base: { id: string; companyName: string; ticker: string; purchasePrice: number; currentPrice: number; highestPrice: number },
    ...
    const buffer = cfg.stopLossPct - drop;
    const status: TrackedStock["status"] =
      drop >= cfg.stopLossPct ? "Cut-Loss" :
      loss >= cfg.takeProfitPct ? "Take-Profit" : "Hold";
    return { ...base, status, dropFromPeak: drop, lossFromPurchase: loss, bufferToStopLoss: buffer, updatedAt: new Date().toISOString() };
  };

  const recalcAll = (cfg: Settings) =>
    setStocks(prev => prev.map(s => calcStock(s, cfg)));

  const applySettings = () => {
    setSettings(settingsForm);
    recalcAll(settingsForm);
  ...
  const fetchPrice = async (id: string, ticker: string) => {
    ...
    try {
      ...
      setStocks(prev => prev.map(s => {
        if (s.id !== id) return s;
        return calcStock({ ...s, currentPrice: currentPrice ?? s.currentPrice, highestPrice: highestPrice ?? s.highestPrice });
      }));
    } catch (e: unknown) {
    ...
    <div style={{ padding: "24px 32px", maxWidth: 1280, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, paddingBottom: 8, borderBottom: "1px solid #21262D" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button
            onClick={() => { setSettingsForm(settings); setShowSettings(!showSettings); }}
            title="손절/익절 기준 설정"
            style={{ ...btnIcon, background: showSettings ? "#21262D" : "transparent" }}
          >⚙️</button>
      ...
      {showSettings && (
        <div style={{ background: "#161B22", border: "1px solid #30363D", borderRadius: 12, padding: "20px 24px", marginBottom: 16 }}>
          <div style={{ fontWeight: 700, color: "#E6EDF3", marginBottom: 4 }}>⚙️ 리스크 관리 기준 설정</div>
          <div style={{ color: "#8B949E", fontSize: "0.8rem", marginBottom: 16 }}>설정 변경 시 전체 포트폴리오가 즐시 재계산됩니다.</div>
          <div style={{ display: "flex", gap: 24, flexWrap: "wrap", marginBottom: 16 }}>
            <div>
              <div style={{ fontSize: "0.75rem", color: "#F85149", fontWeight: 600, marginBottom: 6 }}>🔴 손절 기준 (고점 대비 하락률)</div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <input
                  type="number" min={1} max={50} value={settingsForm.stopLossPct}
                  onChange={e => setSettingsForm(p => ({ ...p, stopLossPct: Number(e.target.value) }))}
                  style={{ ...inputSt, width: 80, textAlign: "center" }}
                />
                <span style={{ color: "#8B949E" }}>%</span>
                <span style={{ color: "#484F58", fontSize: "0.78rem" }}>(현재: -{settings.stopLossPct}%)</span>
              </div>
            </div>
            <div>
              <div style={{ fontSize: "0.75rem", color: "#3FB950", fontWeight: 600, marginBottom: 6 }}>🟢 익절 기준 (매수가 대비 수익륙)</div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <input
                  type="number" min={1} max={200} value={settingsForm.takeProfitPct}
                  onChange={e => setSettingsForm(p => ({ ...p, takeProfitPct: Number(e.target.value) }))}
                  style={{ ...inputSt, width: 80, textAlign: "center" }}
                />
                <span style={{ color: "#8B949E" }}>%</span>
                <span style={{ color: "#484F58", fontSize: "0.78rem" }}>(현재: +{settings.takeProfitPct}%)</span>
              </div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={applySettings} style={btnGreen}>적용</button>
            <button onClick={() => setShowSettings(false)} style={btnCancel}>취소</button>
            <button onClick={() => setSettingsForm(DEFAULT_SETTINGS)} style={{ ...btnCancel, color: "#8B949E", fontSize: "0.78rem" }}>기본값 복원</button>
          </div>
        </div>
      )}
      ...
      {(showForm || editId) && (
        <div style={{ background: "#161B22", border: "1px solid #30363D", borderRadius: 12, padding: "20px 24px", marginBottom: 20 }}>
          <div style={{ fontWeight: 700, color: "#E6EDF3", marginBottom: 4 }}>{editId ? "종목 수정" : "새 종목 추가"}</div>
          <div style={{ color: "#484F58", fontSize: "0.78rem", marginBottom: 14 }}>손절 -{settings.stopLossPct}% / 익절 +{settings.takeProfitPct}% 기준 적용</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12, marginBottom: 16 }}>
            {([
              ...
              { key: "highestPrice",  label: "최고가",       placeholder: "80000 (비울면 현재가 사용)" },
            ] as const).map(f => (
              <div key={f.key}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", gap: 20, marginTop: 12, flexWrap: "wrap" }}>
                    {[
                      ...
                      { label: "고점 대비", value: `-${s.dropFromPeak.toFixed(1)}%`, color: s.dropFromPeak >= settings.stopLossPct ? "#F85149" : s.dropFromPeak >= settings.stopLossPct * 0.6 ? "#E3B341" : "#3FB950" },
                      { label: "수익률",    value: `${s.lossFromPurchase >= 0 ? "+" : ""}${s.lossFromPurchase.toFixed(1)}%`, color: s.lossFromPurchase >= 0 ? "#3FB950" : "#F85149" },
                      { label: "손절 여유", value: `${s.bufferToStopLoss.toFixed(1)}%`, color: s.bufferToStopLoss <= settings.stopLossPct * 0.25 ? "#F85149" : "#8B949E" },
                    ].map(m => (
                      <div key={m.label}>
      ...
      <p style={{ color: "#484F58", fontSize: "0.75rem", textAlign: "center", marginTop: 24 }}>
        ⚙️ 현재 기준: 손절 고점 대비 -{settings.stopLossPct}% / 익절 매수가 대비 +{settings.takeProfitPct}%
        &nbsp;&middot;&nbsp; 우측 상단 ⚙️ 아이콘으로 변경 가능
      </p>
    </div>
  );
}
...
const btnIcon: React.CSSProperties  = { border: "1px solid #30363D", color: "#8B949E", padding: "6px 10px", borderRadius: 8, cursor: "pointer", fontSize: "1rem" };
